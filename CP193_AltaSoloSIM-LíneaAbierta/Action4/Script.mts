...
Sub SeleccionarPlan()
	...
	wait 1
	If DataTable("e_Plan", dtlocalSheet) = "Prepago con Tarifa Única :" Then
		...
		JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Alta Express (Para WALTHER").JavaList("ComboBoxNative$1_2").Select "Todas las subcategorías"
	End If
	wait 1
...
Sub AltaExpress()
	...
	ElseIf  ucase(Financiamiento) = "NO" and TipoAlta = "Alta solo SIM" Then
		...
		JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Alta Express (Para WALTHER").JavaButton("Calcular").Click
		Dim z
		z = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Alta Express (Para WALTHER").JavaStaticText("0.00(st)").GetROProperty("text")
...
Sub NPC()
	...
	For Iterator = filas-1 To 0 Step -1
		...
		End If
	Next
	If DataTable("e_Rol", dtlocalSheet) <> empty Then
		wait 3
		...
		wait 3
		Dim fil, m
		fil =  JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:").GetROProperty("rows")
		For Iter = fil-1 To 0 Step -1
			JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:").SelectRow ("#"&Iter)		
			m = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:").GetCellData("#"&Iter, "#1")	    
	    	m = Instr(1,m,"New attribute description")
	    	
			If m = "759" Then	
			      	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:").ClickCell Iter, 1
			      	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:").SetCellData "#"&Iter, "#1", DataTable("e_Rol", dtlocalSheet)
			      	JavaWindow("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() &"Rol.png", True
					imagenToWord "Seleccionamos Rol",RutaEvidencias() &"Rol.png"
			      	Exit for	    	    
